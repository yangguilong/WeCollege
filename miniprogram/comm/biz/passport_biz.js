...
class PassportBiz extends BaseBiz {
    static async _doLogin() {
        try {
            ...
            const res = await cloudHelper.callCloudSumbitAsync(
                "passport/register",
                userData
            );
            if (res.data && res.data.token) {
                PassportBiz.setToken(res.data.token);
                console.log('登录成功')
            } else {
                console.error(res);
            }
    ...
    static async tryLogin() {
        ...
        console.log("没有登录自动注册并登录");
        try {
            if (!isLogining) {
                isLogining = new Promise(resolve => {
                    wx.showLoading({
                        title: "正在加载 ...",
                    });

                    PassportBiz._doLogin().then(resolve);
                });
            }
            await isLogining;
            isLogining = null;
